...
def main():
    ...
    dataset = OrderSequenceDataset(
        ...
        data_x_meta_path=config.data_x_meta_path,
    ...
    print(f"데이터 로딩 완료: 총 {total_samples}개 (Train {train_size}, Val {val_size}, Test {test_size})")
    ...
    if config.model_type == "cnn":
        ...
        raise ValueError(f"[에러] config.model_type = {config.model_type} 는 올바르지 않은 모델 타입입니다.")
    ...
    print(f"모델 학습 시작: {config.model_type.upper()}")
    train_model(
        model=model,
